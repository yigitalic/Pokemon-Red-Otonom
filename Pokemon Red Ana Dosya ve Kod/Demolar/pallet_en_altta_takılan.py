...
class PokemonAgentPyBoy:
    ...
    def explore_smart(self):
        ...
        if self.last_map_id != -1 and self.last_map_id != map_id:
            ...
            # Kapı Mühürü: Geldiğimiz yönün tersini mühürle
            ...
            else: seal_y += 1 # Varsayılan: Aşağıya dönüşü kapat
        ...
        # [KALICI ENGEL] RETURN_TO_OAK sürecinde Pokemart ve Rival'ın evini engelle
        if self.story_state == "RETURN_TO_OAK":
            if 0x01 not in self.known_walls: self.known_walls[0x01] = set()
            self.known_walls[0x01].add((29, 19)) # Viridian Pokemart
            
            if 0x00 not in self.known_walls: self.known_walls[0x00] = set()
            self.known_walls[0x00].add((27, 23)) # Pallet Rival's House
        ...
        # Pokemart (0x2A) hedef bina değilse bina listesine ekle (yanlış bina mantığı için)
        ...
        if self.starter_picked:
            ...
            if self.story_state == "TO_VIRIDIAN":
                ...
                if map_id != 0x2A:
                    ...
                    hedef_x, hedef_y = 3, 5 # Tezgaha Dogru Yuru
                    ...
                    # Marketten çıkana kadar DOWN basmaya devam et
                ...
                # [MANUEL ROTA] Oak'a dönüş yolu otonom sistemden bağımsız, koordinat odaklı ve aşağı yönlüdür.
                if map_id == 0x2A: 
                    hedef_x, hedef_y = 3, 8 # Marketten çık
                    if self.step_counter % 5 == 0: self.pyboy.button("b")
                elif map_id == 0x24: hedef_x, hedef_y = 7, 1
                elif map_id == 0x25: hedef_x, hedef_y = 3, 8
                elif map_id == 0x01: 
                    # Viridian City: Sadece aşağı ve sola odaklanarak Route 1 girişine git
                    if mem_y < 25: 
                        hedef_x, hedef_y = 21, 25 # Önce ledge hizasına
                    elif mem_y >= 35:
                        ...
                        return # Bu tick'i burada bitir
                    else:
                        hedef_x, hedef_y = 21, 35 # Sonra Route 1 girişine
                    
                    # Pokemart'a geri girmeyi her adımda engelle
                    if 0x01 in self.known_walls: self.known_walls[0x01].add((29, 19))
                elif map_id == 0x0C: 
                    # Rota 1: Sadece aşağı in!
                    if mem_y >= 35:
                    ...
                    # Pallet Town: Direkt laboratuvar kapısına, evlerden uzak dur!
                    hedef_x, hedef_y = 12, 11
                elif map_id == 0x28:
                    hedef_x, hedef_y = 5, 3 # Oak'ın bir kare aşağısı
        ...
        # KESİN KURAL: Pakedi aldıktan sonra dışarıda (Viridian, Route 1, Pallet) ASLA yukarı basma!
        if hasattr(self, 'story_state') and self.story_state == "RETURN_TO_OAK":
            if map_id in [0x01, 0x0C, 0x00] and secilen_yon == "up":
