...
class PokemonBrockBot:
    def __init__(self, rom_path):
        ...
        # Story State tabanlı yeni Rota Sistemi
        self.routes = {
            "GET_PARCEL": {
                self.MAP_REDS_HOUSE_2F: [(7, 1)],
                self.MAP_REDS_HOUSE_1F: [(3, 8)],
                self.MAP_PALLET_TOWN: [(10, 0)],
                self.MAP_OAKS_LAB: [(6, 4), (5, 11)], # Oak laboratuvarına getirdiğinde Pokemon seç ve çık
        ...
        # Takılma Sistemi (Stuck Detection)
        self.last_x = -1
        self.last_y = -1
        self.stuck_frames = 0
        self.escape_dir = 'UP'
    ...
    def move_towards(self, target_x, target_y):
        ...
        if self.stuck_frames > 20:
            # Sıkıştık! Kurtulmak için rastgele bir yöne git
            if self.stuck_frames == 21:
                self.escape_dir = random.choice(['UP', 'DOWN', 'LEFT', 'RIGHT'])
                print(f"[STUCK] {curr_x},{curr_y} konumunda sıkışıldı. {self.escape_dir} yönüne kaçılıyor!")
            self.press(self.escape_dir, 15)
            if self.stuck_frames > 40:
    ...
    def play(self):
        print("[Bot] Başlatıldı. Mimari: Checkpoint + Story State")
        ...
        while self.pyboy.tick():
            ...
            if self.current_route_index < len(current_map_routes):
                ...
                reached = self.move_towards(target[0], target[1])
                if reached:
...
if __name__ == "__main__":
    ...
    rom_path = os.path.join(script_dir, "Pokemon - Red Version (USA, Europe) (SGB Enhanced).gb")
    bot = PokemonBrockBot(rom_path)
    bot.play()
